...
# 加载停用词词典
stopwords = {}
with open(r'资料/XLNet-Chatbot-master/chatbot/stopword.txt', 'r', encoding='utf-8') as fr:
    for word in fr:
...
class clf_model:
    ...
    def train(self):
        ...
        """
        # 从excel文件读取训练样本
        d_train = pd.read_excel("资料/XLNet-Chatbot-master/chatbot/data_train.xlsx")
        # 对训练数据进行预处理
        d_train.sentence_train = d_train.sentence_train.apply(self.fun_clean)
        # print(d_train.sentence_train[:10])
